    ...
    <div class="row">
              ...
              <tbody>
                @foreach($students as $student)
                  <tr>
                    ...
                    <td scope="row">
                      <a href="{{ route('students.profil', ['id'=>$student->id]) }}" title="Profil {{ $student->first_name . ' ' . $student->last_name }}">
                        {{ $student->first_name . ' ' . $student->last_name }}
                      </a>
                    </td>
                    <td class="text-center">{{ $student->code }}</td>
                    <td class="text-center">
                      <a href="{{ route('students.delete', ['id' => $student->id]) }}" class="text-danger" title="Vymazať {{ $student->first_name . ' ' . $student->last_name }}">
      ...
      <div class="col-lg-4 pt-3 pb-3">
        <div class="form-group text-right">
          <button class="btn btn-sm btn-success" id="select-all" data-select="students[]">Vybrať všetko</button>
        </div>
