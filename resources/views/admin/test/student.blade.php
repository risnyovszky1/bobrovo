    ...
    <div class="admin-panel pt-2 pb-2">
            ...
            <div class="col-lg-4">
                ...
                <div class="progress">
                    <div class="progress-bar progress-bar-striped {{ $bg }}" role="progressbar"
                         style="width: {{ $val }}%;" aria-valuenow="{{ $val }}" aria-valuemin="0"
                        ...
                        @if ($answers[$item->id] == $item->answer)
                            @switch($item->type)
                                ...
                                <div class="bg-white border rounded border-success mb-2 pl-3 pr-3 pt-2 pb-2 shadow">
                                    <i class="fas fa-check text-success"></i> <img
                                            src="{{ getQuestionsAnswerText($item, $answers[$item->id]) }}" alt=""
                                            class="d-inline-block"> <span class="text-muted">(odpoveď študenta)</span>
                                </div>
                                @break
                                @case(5)
                                ...
                                @default
                                <div class="bg-white border rounded border-success mb-2 pl-3 pr-3 pt-2 pb-2 shadow">
                                    <i class="fas fa-check text-success"></i> {{ getQuestionsAnswerText($item, $answers[$item->id]) }}
                                    <span class="text-muted">(odpoveď študenta)</span>
                                </div>
                            @endswitch
                        @else
                            @switch($item->type)
                                @case(4)
                                <div class="bg-white border rounded border-danger mb-2 pl-3 pr-3 pt-2 pb-2 shadow">
                                    <i class="fas fa-times text-danger"></i> <img
                                            src="{{ getQuestionsAnswerText($item, $answers[$item->id]) }}" alt=""
                                            class="d-inline-block"> <span class="text-muted">(odpoveď študenta)</span>
                                </div>
                                <div class="bg-white border rounded border-success mb-2 pl-3 pr-3 pt-2 pb-2 shadow">
                                    <i class="fas fa-check text-success"></i> <img
                                            src="{{ getQuestionsAnswerText($item, $item->answer) }}" alt=""
                                            class="d-inline-block"> <span class="text-muted">(správna odpoveď)</span>
                                </div>
                                @break
                                @case(5)
                                <div class="bg-white border rounded border-secondary mb-2 pl-3 pr-3 pt-2 pb-2 shadow">
                                ...
                                @default
                                <div class="bg-white border rounded border-danger mb-2 pl-3 pr-3 pt-2 pb-2 shadow">
                                    <i class="fas fa-times text-danger"></i> {{ getQuestionsAnswerText($item, $answers[$item->id]) }}
                                    <span class="text-muted">(odpoveď študenta)</span>
                                </div>
                                <div class="bg-white border rounded border-success mb-2 pl-3 pr-3 pt-2 pb-2 shadow">
                                    <i class="fas fa-check text-success"></i> {{ getQuestionsAnswerText($item, $item->answer) }}
        ...
        <div class="row">
            <div class="col-lg-8 pb-3">
                <a href="{{ route('test.result', ['test' => $test->id]) }}" class="btn btn-primary shadow"><i
                            class="fas fa-arrow-circle-left"></i> Späť na výsledky testu</a>
            </div>
        </div>
